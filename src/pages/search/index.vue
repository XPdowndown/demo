  ...
  <div>
    ...
    <div class="main">
      <div class="py-container">
        ...
        <!--selector-->
        <SearchSelector @tradesMarkIn="tradesMarkInfo" @getAttrAndAttrValue="getAttrAndAttrValue" />
        ...
        <div class="details clearfix">
            ...
            <div class="navbar-inner filter">
              <ul class="sui-nav">
                <li :class="{ active: isOne }" @click="sort(1)">
                  <a>综合<span v-show="isOne && isDesc">⬇</span>
              ...
              <li class="yui3-u-1-5" v-for="goods in goodsList" :key="goods.id">
                <div class="list-wrap">
                  ...
                  <div class="price">
                    <strong>
                      <em>¥</em>
                      <i style="margin-left: 10px">{{ goods.price }}</i>
                    </strong>
                  </div>
                  <div class="attr">
          ...
          <!-- 分页器 -->
          <!-- <div class="fr page">  </div> -->
          <Pagination :pageNo="searchParams.pageNo" :total="total" :continues="5" :pageSize="searchParams.pageSize" @getPageNo="getPageNo">
          </Pagination>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    // 自定义事件，接受分页组件传过来的当前页数
    getPageNo(pageNo) {
      this.searchParams.pageNo = pageNo;
      this.getDate();
    },
    // 由于search模块要多次向服务器请求数据，因此将以下代码封装成一个函数比较好
    getDate() {
    ...
    // 绑定自定义事件的回调,用来接受子组件绑定的自定义事件传过来的品牌信息数据
    tradesMarkInfo(trades) {
      // 接受了子组件传过来的数据后，就需要整理参数发请求了
      this.searchParams.trademark = `${trades.tmId}:${trades.tmName}`;
      this.getDate();
    },
    // 点击 x 后清除品牌信息
    removeTradeMark() {
    ...
    // 自定义事件回调，用来接受子组件传送过来的属性数据
    getAttrAndAttrValue(attr, attrValue) {
      let props = `${attr.attrId}:${attrValue}:${attr.attrName}`;
      // 接收完数据之后，一样的套路，接下来又是整理参数发请求了
      // 将数据存到数组之时，要进行判断，重复的数据就不要再添加到数组中去了
    ...
    sort(flag) {
      ...
      if (flag == originFlag) {
        ...
        // newOrder = `${originFlag == 1 ? '2' : '1'}:${originSortType}`
        newOrder = `${flag}:${originSortType}`;
      }
